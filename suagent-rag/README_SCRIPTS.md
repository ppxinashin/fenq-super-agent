...
├── logs/                 # 日志目录
│   ├── suagent-rag.log   # 主日志文件
│   └── suagent-rag.pid   # 进程 PID 文件
...
# 搜索错误
grep -i "error" logs/suagent-rag.log
...
# 方法 2: 使用 conda run
conda run -n suagent-rag pip install -r requirements.txt
...
# 查看进程树
pstree -p $(cat logs/suagent-rag.pid)
```
...
# 清空日志（谨慎使用）
> logs/suagent-rag.log

# 删除旧的备份
rm -f logs/suagent-rag.log.*.gz
```
...
- **查看日志**: `tail -f logs/suagent-rag.log`
- **检查环境**: `./check_env.sh`
- **查看状态**: `./status.sh`
