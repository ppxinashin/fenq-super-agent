...
# 配置变量
CONDA_ENV_NAME="suagent-server"
LOG_DIR="logs"
LOG_FILE="${LOG_DIR}/suagent-server.log"
PID_FILE="${LOG_DIR}/suagent-server.pid"
...
nohup conda run --no-capture-output -n "${CONDA_ENV_NAME}" python start_server.py >> "${LOG_FILE}" 2>&1 &
